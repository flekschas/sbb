        ...
        link: function (scope, element) {
          ...
          scope.download = function () {
            if (scope.fileSaverSupport) {
              try {
                var output = '<?xml version="1.0" encoding="utf-8"?><!-- Downlo' +
                  'aded from the Semantic Body Browser (http://sbb.cellfinder.o' +
                  'rg). For term of usage please refer to http://sbb.cellfinder' +
                  '.org/about#copyright --><!DOCTYPE svg PUBLIC "-//W3C//DTD SV' +
                  'G 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd' +
                  '">' + jQuery('<p>').append($svg.clone()).html();

                var blob = new Blob([output], {
                  type: 'image/svg+xml;charset=utf-8'
                });
                saveAs(blob, scope.view.name
                  .replace(/-/g, ' ')
          ...
          fn.show = function () {
            ...
            title.innerHTML = scope.view.name
              .replace(/-/g, ' ')
              .replace(/\w\S*/g, function (txt) {
            ...
            // Add copyright
            var copyright = document.createElementNS(
              'http://www.w3.org/2000/svg',
              'desc'
            );
            ...
            if (scope.view.holdCopyright) {
              copyright.innerHTML = copyright.innerHTML +
                'Copyright (C) Semantic Body Browser. ';
            }
          ...
          fn.setSemantics = function () {
            ...
            paper
              ...
              .setAttribute(
                'about',
                scope.ontId.split('_')[0].toLowerCase() + ':' + scope.ontId
              );
          };
          ...
          fn.error = function () {
            ...
            paper
              .path(
                'M29.225,23.567l-3.778-6.542c-1.139-1.972-3.002-5.2-4.141-7.172' +
                'l-3.778-6.542c-1.14-1.973-3.003-1.973-4.142,0L9.609,9.853c-1.1' +
                '39,1.972-3.003,5.201-4.142,7.172L1.69,23.567c-1.139,1.974-0.20' +
                '7,3.587,2.071,3.587h23.391C29.432,27.154,30.363,25.541,29.225,' +
                '23.567zM16.536,24.58h-2.241v-2.151h2.241V24.58zM16.428,20.844h' +
                '-2.023l-0.201-9.204h2.407L16.428,20.844z'
              )
              .transform(
                't' +
                (($containerWidth / 2) - 15) +
            ...
            for (var i = 0; i < len; ++i) {
              ...
              for (var k = scope.ilu.groups[i].elements.length - 1; k >= lastGroup; --k) {
                if (typeof scope.ilu.groups[i].elements[k] === 'number') {
                  ...
                  if (move) {
                    move = false;
                    vectorElements[scope.ilu.groups[i].elements[k]]
                      .node
                      .parentNode
                      .insertBefore(
                        groups[id][lastGroup].node,
                        vectorElements[scope.ilu.groups[i].elements[k]].node.nextSibling
                      );
                  }
                  ...
                  if (move) {
                    move = false;
                    vectorElements[units[scope.ilu.groups[i].elements[k]]]
                      .node
                      .parentNode
                      .insertBefore(
                        groups[id][lastGroup].node,
                        vectorElements[units[scope.ilu.groups[i].elements[k]]].node.nextSibling
                      );
                  }
          ...
          fn.colorExpression = function (gene) {
            ...
            for (var i in oldColoredUnits) {
              if (coloredUnits.indexOf(oldColoredUnits[i]) === -1) {
          ...
          fn.resetExpressioncolours = function () {
            ...
            for (var i = coloredUnits.length; i >= 0; --i) {
              fn.reset(coloredUnits[i]);
            }
          ...
          fn.click = function (target) {
            if (!scope.heatmap &&
                (target.getAttribute('data-id') || target.getAttribute('data-group'))) {
              scope.setActiveUnit({
                unit: target.getAttribute('data-id'),
          ...
          fn.showZoomDialog = function (x, y) {
            if (x && y) {
              ...
              scope.zoomDisabled = !scope.isZoomable({
                unit: scope.activeUnit
              });
              scope.ontId = scope.getOntId({
                unit: scope.activeUnit
              });
              if (scope.ontId) {
                $http
            ...
            if (!scope.ilu) {
              ...
              if (typeof units[id] !== 'undefined') {
                if (Object.prototype.toString.call(units[id]) === '[object Array]') {
                  if ((fade || (scope.activeUnit && scope.isUnitOverlayed({unit: scope.activeUnit}))) &&
                      !scope.heatmap) {
                    fn.fadeOutElements();
                  }
                  ...
                  while (i--) {
                    if (typeof units[id][i] === 'number') {
                      if (typeof scope.ilu.elements[units[id][i]].stroke === 'undefined' &&
                          scope.ilu.elements[units[id][i]].fill === 'none') {
                  ...
                  if (typeof scope.ilu.elements[units[id]].stroke === 'undefined' &&
                      scope.ilu.elements[units[id]].fill === 'none') {
                    // Unit currently has no color
                    if (scope.heatmap && color === '#ff8a59') {
            ...
            if (!scope.ilu) {
              ...
              if (typeof units[id] !== 'undefined') {
                if (Object.prototype.toString.call(units[id]) === '[object Array]') {
                  ...
                  while (i--) {
                    if (typeof units[id][i] === 'number') {
                      if (scope.heatmap && coloredUnits.indexOf(id) >= 0) {
                        vectorElements[units[id][i]].node.setAttribute('class', '');
                      } else {
                        vectorElements[units[id][i]].attr({
          ...
          fn.autoZoom = function (id) {
            ...
            if (unitAbsSize.width < 0.04 &&
                unitAbsSize.height < 0.1 ||
                (unitAbsSize.height < 0.04 && unitAbsSize.width > 0.1)) {
          ...
          var hammerDoubleTap = new Hammer.Tap({
          ...
          if (isMobile.any) {
            var hammerPinch = new Hammer.Pinch({
                ...
                hammerRotate = new Hammer.Rotate({
                  threshold: 0
                });
